...
def writeLog(RL_params, rst_path, sim_path, clean = False):
    ...
    if clean == True:
        ...
        for each_file in results_list:
            ...
            if os.path.isdir(absolute_path) and len(os.listdir(absolute_path)) == 0 and int(each_file[:8]) < int(current_time[:8]):
                ...
                # shutil.move(absolute_path, trashcan_path)
                
                    

def trainAgent(RL, rst_path, sim_path):
    scatter = []
    for episode in range(SIM_ITERATIONS): 
        ...
        while step < env.duration:
            ...
            if env.isCheckpoint():
                ...
                env.prolongTL(a)
                RL.writeMemoryRecord(o, s, a, r)
                RL.writeLearnLog(episode, step, s, o, a, r)
            else:
                pass
