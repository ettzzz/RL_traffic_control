...
from global_var import trainCmd, TLDB, green_states, SIM_DURATION

IC_list = TLDB['IC_name'].unique()
                
class myTraci():

    def __init__(self, DURATION = SIM_DURATION, sumo_cmd = trainCmd):
        self.conn = traci
        ...
        for each_ic in self.intersections:
            ...
            self.lanes[each_ic].sort(key = lambda x:x[-1])
    ...
    def getCurrentLanePair(self):
        ...
        if a <= 1: # straight/right-turning phase
    ...
    def calWaitingTime(self):
        for each_ic in self.intersections:
            ...
            self.waitingtime[each_ic].append(self.getNetworkWaitingTime())
        ...
        for each_ic in self.intersections:
            ...
            if self.getTL() in green_states:
                ...
                if t0 == self.tl_params['minGrT'] - 1:
                    self.cache_vars['cache_d1'][self.intersection] = self.getNetworkWaitingTime('red')
                if t0 == 0:
                    self.cache_vars['cache_d2'][self.intersection] = self.getNetworkWaitingTime('red')
